...
import {
  Truck, Plus, Search, RefreshCw, Edit3, Trash2, X,
  CheckCircle, XCircle, AlertTriangle, Loader2, Phone, MapPin, User
} from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onCancel}>
      ...
      >
        <div className="flex flex-col items-center text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Xác nhận xóa nhà cung cấp</h3>
          ...
          <p className="text-slate-500 text-xs mb-6">
            Lưu ý: Hành động này có thể bị lỗi nếu nhà cung cấp đã có các phiếu nhập kho đi kèm.
          </p>
...
function SupplierFormModal({ isOpen, mode, supplier, onSubmit, onClose, loading }) {
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-800">
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl border ${mode === 'create'
              ...
              <Truck className={`h-5 w-5 ${mode === 'create' ? 'text-pink-400' : 'text-sky-400'}`} />
            </div>
            <div>
              ...
              <p className="text-xs text-slate-500">
                {mode === 'create' ? 'Đăng ký nhà cung cấp nguyên liệu' : `Đang sửa: ${supplier?.supplier_name}`}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-7 py-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">
              Tên đơn vị nhà cung cấp <span className="text-red-400">*</span>
            </label>
            <input
              type="text"
              ...
              placeholder="vd: Hợp tác xã Rau sạch Đà Lạt..."
              className={`w-full px-4 py-3 bg-slate-900/70 border rounded-xl text-white placeholder-slate-600 focus:outline-none focus:ring-1 transition-all duration-200
                ${errors.supplier_name ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500' : 'border-slate-800 focus:border-pink-500 focus:ring-pink-500'}`}
            />
            {errors.supplier_name && (
              <p className="mt-1.5 text-xs text-red-400 flex items-center gap-1">
                <XCircle className="h-3 w-3" /> {errors.supplier_name.message}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 bg-slate-900/70 border rounded-xl text-white placeholder-slate-650 focus:outline-none focus:ring-1 transition-all duration-200
                ${errors.contact_name ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500' : 'border-slate-800 focus:border-pink-500 focus:ring-pink-550'}`}
            />
            {errors.contact_name && (
              <p className="mt-1.5 text-xs text-red-400 flex items-center gap-1">
                <XCircle className="h-3 w-3" /> {errors.contact_name.message}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 bg-slate-900/70 border rounded-xl text-white placeholder-slate-655 focus:outline-none focus:ring-1 transition-all duration-200
                ${errors.phone ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500' : 'border-slate-800 focus:border-pink-500 focus:ring-pink-550'}`}
            />
            {errors.phone && (
              <p className="mt-1.5 text-xs text-red-400 flex items-center gap-1">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 bg-slate-900/70 border rounded-xl text-white placeholder-slate-655 focus:outline-none focus:ring-1 transition-all duration-200
                ${errors.address ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500' : 'border-slate-800 focus:border-pink-500 focus:ring-pink-550'}`}
            />
            {errors.address && (
              <p className="mt-1.5 text-xs text-red-400 flex items-center gap-1">
                <XCircle className="h-3 w-3" /> {errors.address.message}
              </p>
            )}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-900">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-white font-medium shadow-lg transition-all duration-200 disabled:opacity-50 text-sm flex items-center gap-2
                ${mode === 'create'
                  ? 'bg-pink-600 hover:bg-pink-500 shadow-pink-600/20 hover:shadow-pink-500/30'
...
export default function SupplierManagement() {
  ...
  // Modals state
  const [formModal, setFormModal] = useState({ open: false, mode: 'create', supplier: null });
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setToast({ type: 'error', message: 'Không thể tải danh sách nhà cung cấp.' });
    } finally {
      setLoading(false);
    }
  }, [searchQuery]);
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (formModal.mode === 'create') {
        ...
        const response = await axios.put(`${API_BASE}/suppliers/${formModal.supplier.id}`, data);
        setToast({ type: 'success', message: response.data.message });
      }
      setFormModal({ open: false, mode: 'create', supplier: null });
      fetchSuppliers();
    } catch (err) {
      const msg = err.response?.data?.error || 'Có lỗi xảy ra khi lưu nhà cung cấp.';
  ...
  const handleDeleteConfirm = async () => {
    setActionLoading(true);
    try {
      const response = await axios.delete(`${API_BASE}/suppliers/${deleteModal.supplier.id}`);
      setToast({ type: 'success', message: response.data.message });
      setDeleteModal({ open: false, supplier: null });
      fetchSuppliers();
    } catch (err) {
      const msg = err.response?.data?.error || 'Có lỗi xảy ra khi xóa nhà cung cấp.';
      setToast({ type: 'error', message: msg });
    } finally {
      setActionLoading(false);
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2.5 bg-pink-500/10 rounded-xl border border-pink-500/20 shadow-[0_0_20px_rgba(236,72,153,0.1)]">
          ...
          <p className="text-slate-400 mt-1 ml-[52px]">
            Quản lý thông tin liên hệ và địa chỉ của các đơn vị cung cấp nguyên liệu thực phẩm
          </p>
        </div>
        <button
          onClick={() => setFormModal({ open: true, mode: 'create', supplier: null })}
          className="flex items-center gap-2 px-5 py-3 rounded-xl bg-pink-600 hover:bg-pink-500 text-white font-medium shadow-lg shadow-pink-600/20 transition-all duration-200 hover:shadow-pink-500/30 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-slate-950/70 border border-slate-800 focus:border-pink-500 focus:ring-1 focus:ring-pink-500 rounded-xl text-white placeholder-slate-500 focus:outline-none transition-all duration-200"
          />
        </div>
        {/* Refresh */}
        <button
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-950/50">
                <th className="py-4.5 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Nhà cung cấp</th>
                <th className="py-4.5 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Người liên hệ</th>
                <th className="py-4.5 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Số điện thoại</th>
                <th className="py-4.5 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Địa chỉ</th>
                ...
                <tr>
                  <td colSpan="6" className="py-16 text-center text-slate-500">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <span className="text-sm font-medium">Đang tải nhà cung cấp...</span>
                    </div>
                  </td>
                </tr>
              ) : suppliers.length === 0 ? (
                <tr>
                  <td colSpan="6" className="py-16 text-center text-slate-500">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <span className="text-sm font-medium">Không tìm thấy nhà cung cấp nào</span>
                    </div>
                  </td>
                </tr>
              ) : (
                suppliers.map((sup) => (
                  <tr key={sup.id} className="hover:bg-slate-900/30 transition-colors duration-150">
                    <td className="py-4.5 px-6 text-sm">
                      <div className="font-bold text-white">{sup.supplier_name}</div>
                      <div className="text-slate-500 text-2xs font-mono">ID: #{sup.id}</div>
                    </td>
                    ...
                    <td className="py-4.5 px-6 text-sm text-slate-500">{formatDate(sup.created_at)}</td>
                    <td className="py-4.5 px-6 text-sm text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => setFormModal({ open: true, mode: 'edit', supplier: sup })}
                        ...
                        <button
                          onClick={() => setDeleteModal({ open: true, supplier: sup })}
      ...
      <SupplierFormModal
        ...
        onClose={() => setFormModal({ open: false, mode: 'create', supplier: null })}
